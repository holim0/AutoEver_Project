...
const Header = () => {
  ...
  return (
    <header
      className={cn(
        'sticky top-0 z-[100] px-[var(--side-padding)] transition-shadow',
        isPageScrolled && 'shadow-sm'
      )}
    >
      <div className="flex relative max-w-[var(--max-width)] h-[var(--header-height)]">
        ...
        <nav className="-mr-[20px] flex-1 h-full">
              ...
              <li className="mx-[16px] my-0" key={item.id}>
                <Link
                  className={cn(
                    ...
                    item.isCurPage &&
                      'before:absolute before:bottom-0 before:left-0 before:h-1 before:w-full before:bg-mint-900 before:content-[""]'
                  )}
                  href={item.href}
                >
                  {item.name}
